...
using AppLimit.CloudComputing.SharpBox.DropBox.Authorization;
...
namespace AppLimit.CloudComputing.SharpBox.DropBox.Logic
{
    internal class DropBoxApplication
    {
        ...
        {            
            ...
            var authorizationLink = OAuthService.GetAuthorizationUrl(serviceContext, requestToken);
            
            // do the authorization via web
            var auth = new DropBoxWebAuthorization();

            auth.ShowHidden = ShowHidden;

            if (!auth.Authorize(authorizationLink, userName, password, _dropBoxCallbackUrl))
                return null;

            // exchange a request token for an access token
            var accessToken = OAuthService.GetAccessToken(serviceContext, consumerContext, requestToken);
