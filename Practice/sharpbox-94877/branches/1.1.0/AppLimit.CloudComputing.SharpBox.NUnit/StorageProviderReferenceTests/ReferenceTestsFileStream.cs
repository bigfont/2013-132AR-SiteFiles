...
namespace AppLimit.CloudComputing.SharpBox.NUnit.StorageProviderReferenceTests
{
    partial class GenericReferenceTests : ReferenceBaseTest 
    {
        ...
        void TestFileOperationProgressChanged(ICloudFileSystemEntry file, long currentbytes, long sizebytes, Object progressContext)
        {
            long iPercentage = (100 * currentbytes) / sizebytes;
            
            if (sizebytes == 0)
                Console.WriteLine("[{2}] Progress: {1} {0} Bytes", currentbytes, DateTime.Now.ToLongTimeString(), Thread.CurrentThread.ManagedThreadId);
            else
                Console.WriteLine("[{4}] Progress: {3} {0} of {1} ({2}%)", currentbytes, sizebytes, iPercentage, DateTime.Now.ToLongTimeString(), Thread.CurrentThread.ManagedThreadId);
        }
        ...
        public void FileUploadFileRoutine(String fileNameFromClientData)
        {
            // set the names
        ...
        {
            FileUploadFileRoutine("DirectUploadTest.bmp");
            FileUploadFileRoutine("Glück.pdf");
        }
        
        [Test()]
        public void FileVerifyFileAttributes()
        {                       
            // WebDAV currently does not support special properties            
            if (Configuration is WebDavConfiguration)
                return;

            // CIFS also does not support special properties
            if (Configuration is CIFSConfiguration)
                return;
