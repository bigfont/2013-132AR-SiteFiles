...
namespace AppLimit.CloudComputing.SharpBox.Common.Net.oAuth.Impl
{
    ...
    {       
        ...
        static protected string unreservedChars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_.~";
        ...
        static public string UrlEncode(string value) 
        {
            ...
            else
            {
                ...
                {
                    if (unreservedChars.IndexOf(value[i]) == -1)
                    {
                        // character needs encoding
                        byte[] characterBytes = Encoding.UTF8.GetBytes(value[i].ToString());
        ...
        public virtual string GenerateNonce() 
        {
            ...
            // and will remove some special characters
            nonce = nonce.Replace("{", "");
            nonce = nonce.Replace("}", "");
            nonce = nonce.Replace("-", "");
