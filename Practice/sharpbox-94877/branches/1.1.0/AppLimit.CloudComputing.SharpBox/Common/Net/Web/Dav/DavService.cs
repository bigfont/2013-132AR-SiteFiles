...
namespace AppLimit.CloudComputing.SharpBox.Common.Net.Web.Dav
{
    internal class DavService : WebRequestService
    {
        ...
        public WebRequest CreateWebRequestPROPFIND(String url, ICredentials credentials)
        {
            ...
            WebRequest request = CreateWebRequest(url, ValidRequestMethod.PropFind, credentials, null);
            ...
            {
                // copy the data
                StreamHelper.CopyStreamData(contentStream, requestStream, null, null);                
            }
        ...
        public WebRequest CreateWebRequestPUT(String url, ICredentials credentials)
        {
            // create the request
            HttpWebRequest request = CreateWebRequest(url, ValidRequestMethod.Put, credentials, null) as HttpWebRequest;
        ...
        public override WebRequest CreateWebRequest(string url, ValidRequestMethod method, ICredentials credentials, object context)
        {
            // quote the string in the right way
            String quotedurl = HttpUtilityEx.GenerateEncodedUriString(new Uri(url));

            // create base request
            WebRequest request = base.CreateWebRequest(quotedurl, method, credentials, context, DavPreparation) as HttpWebRequest;
...
#if !WINDOWS_PHONE
            else if (hrequest.Method == ValidRequestMethod.Put.ToString().ToUpper())
            {
                hrequest.AllowWriteStreamBuffering = false;
            }
#endif
        }
