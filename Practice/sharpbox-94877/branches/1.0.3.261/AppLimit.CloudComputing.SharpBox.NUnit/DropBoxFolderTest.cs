    ...
    public class DropBoxFolderTest : DropBoxBaseTest
    {
        ...
        public void RenameFileTest()
        {
            ...
            if (Environment.OSVersion.Platform == PlatformID.Win32NT)
			{
                ...
                Assert.IsNotNull(CloudStorage.UploadFile("C:\\windows\\" + fileName, publicFolder));
			}
            else
			{
				fileName = "which"; 
                Assert.IsNotNull(CloudStorage.UploadFile("/usr/bin/"+ fileName, publicFolder));
			}
            
            // 3. Get file object
            ICloudFileSystemEntry fsEntry = CloudStorage.GetFile("/Public/" + fileName, null );
        ...
        [Test()]
        public void GetFolderWithTrailingSlash()
        {
            ICloudDirectoryEntry folder = CloudStorage.GetFolder("/Public/");            
            Assert.IsNotNull(folder);
        }

        [Test()]
        public void GetLongFolder()
        {
            ICloudDirectoryEntry folder = CloudStorage.GetFolder("/Public/1/2/3/4/5");
            Assert.IsNotNull(folder);
        }

        [Test()]
        public void GetLongFolderByParent()
        {
            ICloudDirectoryEntry folder = CloudStorage.GetFolder("/Public/1/2/3");
            Assert.IsNotNull(folder);
        ...
        [Test()]
        public void TryToGetFolderAsFile()
        {
            Boolean bGotexception = false;

            try
            {
                ICloudFileSystemEntry file = CloudStorage.GetFile("/Public", null);
            }
