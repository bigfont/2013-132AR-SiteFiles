...
namespace AppLimit.CloudComputing.SharpBox.NUnit
{
    ...
    public class DropBoxFileStreamTest : DropBoxBaseTest
    {
        ...
        public void DownloadGifComfortTest()
        {
            ...
            // download the file
			if (Environment.OSVersion.Platform == PlatformID.Win32NT)					
            	Assert.IsTrue(CloudStorage.DownloadFile(root, "hhw.gif", "%temp%"));
			else
				Assert.IsTrue(CloudStorage.DownloadFile(root, "hhw.gif", "/tmp"));

            // download the file with second methid
			if (Environment.OSVersion.Platform == PlatformID.Win32NT)					
            	Assert.IsTrue(CloudStorage.DownloadFile("/hhw.gif", "%temp%"));
			else
				Assert.IsTrue(CloudStorage.DownloadFile("/hhw.gif", "/tmp"));
        }
        ...
        public void DownloadNotExistingFile()
        {
            ...
            // download the file
            if (Environment.OSVersion.Platform == PlatformID.Win32NT)
                Assert.IsFalse(CloudStorage.DownloadFile(root, "/MissingFile.missing", "%temp%"));
            else
                Assert.IsFalse(CloudStorage.DownloadFile(root, "/MissingFile.missing", "/tmp"));
        }
