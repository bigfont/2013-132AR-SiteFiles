...
namespace AppLimit.CloudComputing.SharpBox.ClientSample
{
    public partial class Form1 : Form
    {
        ...
        {                        
            ...
            {
                AccountForm dlg = new AccountForm();
                if (dlg.ShowDialog() == DialogResult.OK)
                {
                    dropBoxSecrets = new AccountDatabase(dlg.ConsumerKey, dlg.ConsumerSecret);
                    userAccount = new AccountDatabase(dlg.User, dlg.Password);
                }
                else
                {
        ...
        void LoadSandboxInTree(object sender, DoWorkEventArgs e)
        {
            ...
            cred.ConsumerKey = dropBoxSecrets.User;
            cred.ConsumerSecret = dropBoxSecrets.Password;
            ...
            if (!_cloudStorage.Open(configuration, cred))
            {
                backgroundWorker1.ReportProgress(1, "Failed to retrieve data from storage provider");
                return;
            }
