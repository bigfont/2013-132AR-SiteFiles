...
namespace AppLimit.CloudComputing.SharpBox.StorageProvider.DropBox.Logic
{
    ...
    {        
        public static String RequestResourceByUrl(String url, IStorageProviderService service, IStorageProviderSession session, out HttpStatusCode netErrorCode)
        {
            return RequestResourceByUrl(url, null, service, session, out netErrorCode);
        }

        public static String RequestResourceByUrl(String url, Dictionary<String, String> parameters, IStorageProviderService service, IStorageProviderSession session, out HttpStatusCode netErrorCode)
        {
            ...
            WebRequest request = svc.CreateWebRequest(url, ValidRequestMethod.Get, null, null, dropBoxSession.Context, (DropBoxToken)dropBoxSession.SessionToken, parameters);

            // get the error code
            WebException ex;

            // perform a simple webrequest 
            Stream s = svc.PerformWebRequest2(request, null, out netErrorCode, out ex);            
            if (s == null)
                return "";
        ...
        private static Boolean BuildFileEntry(BaseFileEntry fileEntry, JsonHelper jh)
        {
            ...
            fileEntry.SetPropertyValue("path", DropBoxPath);            
