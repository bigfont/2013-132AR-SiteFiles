...
{    
    partial class GenericReferenceTests : ReferenceBaseTest
    {
        ...
        [Test()]
        public virtual void FolderCreateAndReadEncodedFileNames()
        {
        ...
        public void FolderRootEnumNumberFolderTest()
        {
            ...
            Assert.IsNotNull(fs);
            fs.GetDataTransferAccessor().Transfer(new MemoryStream(1), nTransferDirection.nUpload);
        ...
        public void FolderGetRootTest()
        {
            ...
            // enum child
            foreach (ICloudDirectoryEntry f in root)
            {
                Assert.IsNotNull(f);
            }
        }
